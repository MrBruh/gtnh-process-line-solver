...
class ViolationCode(str, Enum):
    ...
    PORT_HATCH_MISSING = (
        "port_hatch_missing"  # a connection was made to a multiblock that has no hatch for it
    )
...
class Violation:
    ...
    violation needs it structurally. Today that consumer is the solver, which maps a starved
    machine back to its power net and ranks the layout as having left that net unrouted
    (``solver/core``). Optional,
