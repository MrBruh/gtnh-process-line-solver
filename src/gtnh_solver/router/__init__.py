...
single-channel cap, one route per cell, which the validator independently enforces). Crude:
one channel per cell, fixed net order, item/fluid only (docs/ROADMAP.md).
...
several parallel channels) + cell->block realizability, adds rip-up-and-reroute (so a bad net
order is not a false infeasibility), ME endpoint placement, and the shared-amperage power
*optimization* (multi-source / split / upgrade) beyond Phase 1's size-or-reject
