...
- **A pipe whose two ends share a cell is one block, not two with one leading nowhere.** Since the
  ...
  have a segment, so the router laid a second block beside it just to have one, and that block led
  nowhere. `examples/gtnh-nitrobenzene.json` had two: one on the bottom layer beside the nitrogen
  tank's feed into the ammonia reactor, and one above the water tank's feed into the sulfuric acid
  reactor. A pipe route with no segments is now a valid one-block route (its terminals all on that
  block), so both feeds are one block each and nothing dangles. A cable still needs a segment,
  since its gauge lives on them, so power routing is unchanged.

  **Breaking for a consumer of the layout JSON:** `LAYOUT_RESULT_VERSION` is now 3. No field
  changed, but a v2 consumer that builds a route's blocks from `segments` alone would build nothing
  for a one-block pipe, so it must also take the terminals' cell (`Route.cells()` does both). A
  v2 layout is refused on read, as the versioning rules say, and is regenerated by re-solving.
...
- **An item pipe is now laid at the size its run needs, not always the normal one (#165).** The
  ...
  *insertions* per window, each landing at most one stack in one inventory, nearest first
  (`MTEItemPipe`), so a pipe whose tooltip reads "1 Stacks/2 sec" carries the line five times
  over by item count and still starves the far hammers. Per size, tin makes 1 insertion per 160,
  80, 40 and 20 ticks for tiny to large and 2 per 20 for huge, transcribed from
  `LoaderMetaPipeEntities.ItemPipeBuilder` and pinned by a test that recomputes GT's arithmetic
  (`dataset/pipe_capacity.py`). A run needs one insertion per 40 ticks for every endpoint on its
  crowded side, and one more per extra stack an endpoint moves in that time. That 40 is **not** a
  GT figure: it is the one in-game measurement there is (the plain pipe, one insertion per 40
  ticks, fed one hammer and not two), and it says so where it is defined. Fluid pipes are not
  sized yet and stay normal; the shipped lines' busiest fluid net is a quarter of a normal bronze
  pipe's 120 mB/t.

  A run is sized as a whole, for the point where every stream on its crowded side can meet. On the
  parallel sand line that asks for huge between the hammer stages too, where the maintainer's own
  hand build gets by with large, because there each pipe block touches one producer and one
  consumer; the router cannot build that pairing yet (#164), and one size too big never starves
  anything. Checking that a laid size is enough, rather than choosing one, is the validator's job
  and is #190.

  **Breaking for layout readers: `LayoutResult` is now v2.** `RouteMaterial` gains `size` (tiny,
  small, normal, large, huge), required on a pipe and invalid on a cable, the mirror of `tier`.
  Additive in shape and still a bump, because a consumer that ignores it builds every pipe normal,
  which is exactly the line that failed. A v1 layout is refused on parse, as every other version
  mismatch is, and is regenerated by re-solving its plan. The committed texture manifest now also
  carries the large and huge tin item pipes, re-derived with `tools/derive_small_manifest.py`, and
  their 2.9 names ("Large Tin Item Pipe", "Huge Tin Item Pipe") are pinned from a real 2.9 dump.

- **A dataset dump that declares another schema version is refused, instead of being read as "no
  hatch slots recorded" (#113).** `SCHEMA_VERSION` was declared, exported and parsed into both
  models, and nothing ever compared it: the on-disk dataset is the third versioned contract here,
  alongside the two `ir/` `version` fields and the plan's `schema_version`, and it was the one with
  no gate. `extra="forbid"` covers only half the problem. A *newer* dump usually carries a field
  this build does not know and fails on that; a *stale* one carries nothing extra at all, validates
  clean, and is then used as if it agreed.

  The stale direction is the one that bites, because a field a dump predates is absent rather than
  wrong, and absent reads as "not recorded, impose no constraint" the whole way down: a schema-v1
  dump has no `hatch_slots`, so every body cell of every machine becomes a hatch candidate, routing
  claims the dock cell instead of the casing cell, and `HATCH_CELLS_EXCEEDED` can never fire. This
  was reachable in normal use, since `data/<version>/` dumps are local-only and regenerated on
  demand, so an older one sitting on disk is the expected state rather than a contrived one.

  `load_multiblock_doc` and `load_meta` now refuse any file whose `schema` is not this build's,
  naming the file and both versions and saying what to do (re-run the extractor for this pack, or
  pass `--dataset-version` to pin a dump this build reads). `_meta.json` is read first, so a whole
  stale dump is refused before a single machine file parses. The refusal is a `ValueError`
  (`DatasetSchemaError`), so `gtnh-solve` keeps its documented 0/1/2 exit contract and degrades to
  1x1x1 footprints with a warning rather than taking the run down. The models stay lenient on
